...
0: "schedules"
*/
const SchedulerInstanceTable: React.FC<SchedulerInstanceTableProps> = ({
  schedulerInstances,
  onClick,
}) => {
  ...
  return (
    <table key="table" className="table is-striped is-hoverable is-fullwidth">
      <thead>
        <tr>
          <th style={{ cursor: "pointer", minWidth: 150 }}>
            {t("SchedulerInstanceTable-column-Ip")}
          </th>
          <th style={{ cursor: "pointer", minWidth: 150 }}>
            {t("SchedulerInstanceTable-column-Hostname")}
          </th>
          <th style={{ cursor: "pointer" }}>
            {t("SchedulerInstanceTable-column-BootstrapServers")}
          </th>
          <th style={{ cursor: "pointer" }}>
            {t("SchedulerInstanceTable-column-Topics")}
          </th>
        </tr>
      </thead>
        ...
        {schedulerInstances.map((inst) => {
          return (
            <tr key={`${inst.ip}`} onClick={() => onClick && onClick(inst)}>
              ...
              <td>{inst.hostname.join(", ")}</td>
              <td>{inst.bootstrap_servers}</td>
              <td>{inst.topics.join(", ")}</td>
            </tr>
          );
        })}
      </tbody>
