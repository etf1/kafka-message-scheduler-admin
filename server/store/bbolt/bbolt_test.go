...
func TestBboltStore_add_get_list(t *testing.T) {
	...
	defer func() {
		err := os.Remove(file)
		if err != nil {
			t.Fatalf("unable to delete db file %v: %v", file, err)
		}
	}()
	...
	db.Add("scheduler-1", sch1, sch2)
	db.Add("scheduler-2", sch3, sch4)
	db.Add("scheduler-3", sch1, sch1Bis)
	...
	if len(lst) != 1 {
		t.Fatalf("unexpected result length: %v", len(lst))
	}

	if lst[0].ID() != "schedule-1" || lst[0].Epoch() == 0 || lst[0].Timestamp() == 0 {
		t.Fatalf("unexpected schedule: %v", lst[0])
	}
	...
	if lst[1].ID() != "schedule-1" || lst[1].Epoch() == 0 || lst[1].Timestamp() == 0 {
		t.Fatalf("unexpected schedule: %v", lst[1])
	}
...
func TestBboltStore_delete(t *testing.T) {
	...
	if len(lst) != 1 {
		t.Fatalf("unexpected result: %v", len(lst))
	}
...
loop:
	...
	if counter == -1 {
		t.Fatalf("unexpected behaviour: channel not closed")
	}
	if counter != 0 {
		t.Fatalf("unexpected error length: %v", counter)
	}
