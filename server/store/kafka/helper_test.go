...
func ifelse(cond bool, then int, otherwise int) int {
	if cond {
		return then
	}
	return otherwise
...
func NewKafkaStore(t *testing.T, nbTopic int, nbPartitions []int) (kafka.Store, []string) {
	topics := CreateTopics(t, nbTopic, nbPartitions, "schedules")
	return NewKafkaStoreFromTopics(t, topics), topics
}

func NewKafkaStoreFromTopics(t *testing.T, topics []string) kafka.Store {
	store, err := kafka.NewStore([]kafka.Bucket{{"scheduler-1", GetBootstrapServers(), topics}})
	if err != nil {
...
func AssertMessageEquals(t *testing.T, m1, m2 *confluent.Message) {
	...
	if !bytes.Equal(m1.Key, m2.Key) {
		t.Fatalf("keys not equals, %v != %v", string(m1.Key), string(m2.Key))
	}
...
func CreateTopic(t *testing.T, topic string) {
	...
	results, err := adm.DeleteTopics(ctx, []string{topic}, confluent.SetAdminOperationTimeout(2*time.Second))
	if err != nil {
		t.Fatalf("failed to delete topic %v: %v", topic, err)
	}
	time.Sleep(2 * time.Second)
	for _, result := range results {
		t.Logf("result deletion: %s\n", result)
	}
	...
	results, err = adm.CreateTopics(ctx, []confluent.TopicSpecification{spec}, confluent.SetAdminOperationTimeout(2*time.Second))
	if err != nil {
		t.Fatalf("failed to create topic %v: %v", topic, err)
	}
	time.Sleep(2 * time.Second)
	for _, result := range results {
		t.Logf("result creation: %s\n", result)
	}
}
